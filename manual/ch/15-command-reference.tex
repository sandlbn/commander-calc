...
\ccrunin{Notes}
\begin{ccsteps}
\item The list holds \textbf{at most thirty-two} files, unsorted, and does not
scroll.
...
\item \key{Esc} drops the block, and so does \keyc{Ctrl}{A} again.
\item The anchor may end up below and right of the cursor; the block is the
rectangle between them either way.
\item \menupath{Copy} and \menupath{Clear} act on the block when there is one
and on the cursor cell when there is not.
\end{ccsteps}
...
Sorts whole rows into ascending order on the column the cursor is in, starting
at the row the cursor is on.

\ccrunin{Procedure}
\begin{ccprocedure}[]
\item Put the cursor in the column to sort by, on the first row of data --- not
on the heading.
...
\item The cursor is put on \cellref{A1} afterwards.
\end{ccsteps}
