...
/* The shipped examples open, and their formulas were worked out HERE.
 *
 * These are the files in the release, generated by tools/make_x16s.py
 * without the machine. The point is not that they parse -- it is that the
 * numbers on screen came from this evaluator, so an example using a
 * function or a form this build cannot compile is caught before it ships
 * rather than by whoever opens it.
 */
/* Open one of the shipped examples and insist every cell of every sheet
 * renders as something other than an error.
 *
 * A formula this build cannot work out shows as #CYCLE!, #NAME? or #REF!
 * and looks perfectly deliberate in a generator's output -- the month
 * column of LOAN.X16S shipped reading its own row for exactly that reason.
 * Missing files are skipped: NDQNAT needs a download to build. */
static void example_has_no_errors(const char *name)
{
    ...
    if (x16s_open(name) != ERR_OK)
        return;
    for (sh = 0; sh < wb_sheet_n; ++sh) {
        CHECK_EQ(wb_sheet_switch(sh), ERR_OK);
        cs = wb_cells();
        for (r = 0; r <= cs->max_row; ++r)
            for (c = 0; c <= cs->max_col; ++c) {
                wb_display_text(r, c, b, sizeof b);
                if (b[0] == '#') {
                    fprintf(stderr, "  %s sheet %u r%u c%u -> %s\n",
                            name, (unsigned)sh, (unsigned)r, (unsigned)c, b);
                    CHECK(b[0] != '#');
...
static void test_shipped_examples(void)
{
    char b[WB_TEXT_MAX];

    setup();

    /* LOAN: every instalment reads the rate as $B$2, which is the whole
     * reason this example exists. 12000 * 6% / 12, to the penny. */
    CHECK_EQ(x16s_open("LOAN.X16S"), ERR_OK);
    disp(5, 2, b); CHECK_STR(b, "$60.00");
    /* The month column counts up by reading the row ABOVE. Written to read
     * its own row it is a self-reference, and every one of them comes back
     * #CYCLE! -- which is what shipped until an example was opened. */
    disp(5, 0, b);  CHECK_STR(b, "1");
    disp(6, 0, b);  CHECK_STR(b, "2");
    disp(28, 0, b); CHECK_STR(b, "24");
    disp(5, 1, b); CHECK_STR(b, "$12000.00");
    /* The chain: the second row opens where the first closed. 12000 plus
     * 60 interest less 340 of capital, the payment being 400. */
    disp(5, 3, b); CHECK_STR(b, "$340.00");
    disp(5, 4, b); CHECK_STR(b, "$11720.00");
    disp(6, 1, b); CHECK_STR(b, "$11720.00");

    /* GRADES: two sheets, and every formula on the summary crosses the
     * boundary. Aldridge sat 72, 65, 80 and 58. */
    CHECK_EQ(x16s_open("GRADES.X16S"), ERR_OK);
    CHECK_EQ(wb_sheet_switch(1), ERR_OK);       /* Summary */
    disp(1, 2, b); CHECK_STR(b, "80");          /* MAX  */
    disp(1, 3, b); CHECK_STR(b, "58");          /* MIN  */
    disp(1, 4, b); CHECK_STR(b, "22");          /* ABS(MAX-MIN) */
    disp(1, 5, b); CHECK_STR(b, "TRUE");        /* AND(...) */
    disp(1, 1, b); CHECK_STR(b, "68.8");        /* AVERAGE, one decimal */

    /* SHIFTS: INT and MOD split a run of hours into days and hours. The
     * first shift is six, so no whole day and six left over. */
    CHECK_EQ(x16s_open("SHIFTS.X16S"), ERR_OK);
    disp(1, 2, b); CHECK_STR(b, "6.00");
    disp(1, 5, b); CHECK_STR(b, "0");           /* INT(6/8)  */
    disp(1, 6, b); CHECK_STR(b, "6.00");        /* MOD(6,8)  */

    /* And nothing anywhere in any of them reads as an error. KINDS is the
     * exception on purpose -- it demonstrates the cell kinds, =1/0
     * included. */
    example_has_no_errors("LOAN.X16S");
    example_has_no_errors("GRADES.X16S");
    example_has_no_errors("SHIFTS.X16S");
    example_has_no_errors("INVOICE.X16S");
    example_has_no_errors("RAIN.X16S");
    example_has_no_errors("BUDGET.X16S");
    example_has_no_errors("SALES.X16S");
    example_has_no_errors("PARTS.X16S");
    example_has_no_errors("NDQNAT.X16S");
