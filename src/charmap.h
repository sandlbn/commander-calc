/* charmap.h — turn off cc65's ASCII-to-PETSCII literal translation.
 *
 * The cx16 target rewrites every string and character literal at compile
 * time. Mapping the whole table to itself means literals, file bytes,
 * imported text and the screen font all agree on one encoding.
 *
 * Must be included before any string literal in a translation unit;
 * x16sheet.h does that for every file.
 *
 * Filenames handed to the KERNAL are the one place PETSCII still applies.
 * Uppercase A-Z and the digits are identical in both encodings, so a card
 * file called OVL1.BIN is opened with "OVL1.BIN"; anything outside that
 * range goes through to_petscii() in file_io_x16.c. See
 * docs/design/platform.md.
 */
#ifndef X16S_CHARMAP_H
#define X16S_CHARMAP_H
...
/* If a literal ever stops meaning itself again, fail here at build time
 * rather than months later as a file that parses differently on the two
 * targets. The negative array size is the C89 way to say static_assert. */
typedef char x16s_charmap_is_identity[
    ('\n' == 10 && '\r' == 13 && 'A' == 65 && 'a' == 97 && '"' == 34)
    ? 1 : -1];

#endif /* X16S_CHARMAP_H */
