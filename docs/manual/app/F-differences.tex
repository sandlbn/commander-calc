...
\begin{ccsteps}
\item \textbf{Formatting is bold, borders and column widths.} Those can be
set from the menus. Number formats and alignment are supported by the display
and by both file formats, but arrive from imported files only --- there is no
command to set them.
\item \textbf{One currency symbol, \lit{\$}}, and no thousands separators.
\item \textbf{One date format, \lit{YYYY-MM-DD}.}
\item \textbf{No colours, fonts or cell shading}, and borders are lines
rather than styles: one weight, one colour, no dashes.
