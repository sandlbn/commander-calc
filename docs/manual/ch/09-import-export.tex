...
\fname{.CSV}, in the same thirty-two-entry list \menupath{Open} uses. Choose
...
worksheet you are on, at the cursor; an XLSX replaces the entire workbook.}

\section{Importing a CSV}
\index{CSV!import}

The file is read in at the cursor cell, which becomes the top-left corner of
the imported block. Put the cursor on \cellref{A1} to import at the origin, or
anywhere else to drop the data into a corner of an existing sheet.
