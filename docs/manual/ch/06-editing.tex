...
Drop it & \keyc{Ctrl}{A} again \\
Copy it & \keyc{Ctrl}{C} --- which drops it too \\
Clear it & \key{Del} --- which drops it too \\
Sort it & \menupath{Data > Sort up} or \menupath{Sort down} \\
\end{cctable}

\begin{ccnote}
\keyc{Ctrl}{A} is the only way to drop a block deliberately; \key{Esc} does
nothing here, because it belongs to editing a cell. \menupath{Copy} and
\menupath{Clear} let go of the block by themselves once they have finished
with it. Everything else --- pasting, sorting --- leaves it marked.
\end{ccnote}
