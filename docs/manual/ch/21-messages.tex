...
\msg{end it .X16S} & A hint on the \menupath{Save as} name box. The format is
always native whatever you type; the hint is about the name. \\
\msg{end it .XLSX for Excel, .CSV for plain text} & The same box under
\menupath{Export}, where the extension does decide the format. \\
\msg{Nothing to freeze here} & \menupath{Layout > Freeze} with the cursor too
far down the sheet or too far right --- freezing there would leave nothing to
scroll. \\
\msg{data beside the block will not move} & Before sorting a selection that is
narrower than the data on its rows. \key{Y} sorts the block alone. See
...
extension, and no subdirectories, in this directory. \\
\msg{cannot open that directory} & A directory in the list could not be read
--- the card has changed, or the name is no longer there. The list stays where
it was. \\
